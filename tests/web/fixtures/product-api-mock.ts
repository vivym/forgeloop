...
export const defaultProductApiResponses: ProductApiResponseMap = {
  ...
  [`GET /query/specs-execution-plans?project_id=${projectId}`]: {
    items: [
      { id: spec.id, artifact_type: 'spec', title: specRevision.summary, href: '/specs-plans', updated_at: spec.updated_at },
      {
  ...
  [`GET /query/executions?project_id=${projectId}`]: {
    items: [{ id: execution.id, title: execution.ref.title, status: execution.status, href: `/executions/${execution.id}`, updated_at: execution.updated_at }],
    degraded_sources: [],
  },
  [`GET /query/executions/${execution.id}`]: execution,
  [`GET /query/code-review-handoffs?project_id=${projectId}`]: {
    items: [{ id: codeReviewHandoff.id, title: codeReviewHandoff.ref.title, status: codeReviewHandoff.status, href: `/code-review-handoffs/${codeReviewHandoff.id}` }],
    degraded_sources: [],
  },
  [`GET /query/qa-handoffs?project_id=${projectId}`]: {
    items: [{ id: qaHandoff.id, title: qaHandoff.ref.title, status: qaHandoff.status, href: `/qa-handoffs/${qaHandoff.id}` }],
    degraded_sources: [],
  },
