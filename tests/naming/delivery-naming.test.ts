...
const allowedWorkItemOwnerActorIdReference = (rel: string, context: string): boolean => {
  ...
  const executionPackageOwnerTestPaths = [
    ...
    /^tests\/db\/(?:automation-repository|codex-runtime-drizzle-concurrency|codex-runtime-repository|release-cockpit-queries|release-replay-queries|repository|work-item-delivery-readiness|work-item-delivery-selection|work-item-release-readiness)\.test\.ts$/,
