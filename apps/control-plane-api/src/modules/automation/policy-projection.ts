...
import type { AutomationRuntimeSnapshotPolicyProjectionDto } from './automation.dto';
import { toPolicyProjectionDto } from './automation.dto';
...
export const policyProjectionsByRepoScopeFor = (
  actionRuns: AutomationActionRun[],
): Map<string, AutomationRuntimeSnapshotPolicyProjectionDto> => {
  const policyProjectionsByRepoScope = new Map<string, AutomationRuntimeSnapshotPolicyProjectionDto>();
  for (const actionRun of actionRuns) {
    ...
    const lastKnownGood =
      input.status === 'parse_failed' || input.status === 'unsafe_path'
        ? actionRuns.find((candidate) => {
            const candidateInput = policyInput(candidate);
            if (candidateInput === undefined) {
              return false;
            }
            const candidateScope = canonicalProjectionScope(candidate, candidateInput.repoId);
            return (
  ...
  return policyProjectionsByRepoScope;
};
