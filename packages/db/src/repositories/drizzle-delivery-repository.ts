...
export class DrizzleDeliveryRepository implements DeliveryRepository {
  ...
  private async createCodexSessionTurnUnlocked(turn: CodexSessionTurn): Promise<void> {
    ...
    if ((activeSession.latest_capsule_digest ?? null) !== (turn.expected_input_capsule_digest ?? null)) {
      throw new DomainError('codex_runtime_capsule_stale', `codex_runtime_capsule_stale: Codex session ${turn.codex_session_id} capsule is stale`);
    }
  ...
  async markCodexSessionTurnStale(input: { session_id: string; turn_id: string; now: string }): Promise<void> {
    ...
    await this.db
      ...
      .set({
        status: 'stale',
        outputCapsuleId: null,
        outputCapsuleDigest: null,
  ...
  async createCodexRuntimeCapsule(capsule: CodexRuntimeCapsule): Promise<void> {
    if ((await this.getCodexSession(capsule.codex_session_id)) === undefined) {
      throw new DomainError('workflow_invalid_transition', `workflow_invalid_transition: Codex runtime capsule ${capsule.id} session ${capsule.codex_session_id} does not exist`);
    }
    const sourceTurn = await this.getCodexSessionTurn(capsule.created_from_turn_id);
    if (sourceTurn === undefined || sourceTurn.codex_session_id !== capsule.codex_session_id) {
      throw new DomainError('codex_runtime_capsule_stale', `codex_runtime_capsule_stale: Codex runtime capsule ${capsule.id} source turn is stale`);
    }
    ...
    try {
      ...
      throw new DomainError('workflow_invalid_transition', `workflow_invalid_transition: Codex runtime capsule ${capsule.id} artifact_ref is not an internal artifact ref`);
    }
    if (
      ...
      throw new DomainError('workflow_invalid_transition', `workflow_invalid_transition: Codex runtime capsule ${capsule.id} artifact_ref does not match the capsule identity`);
    }
    const [existingForSequence] = await this.db
      .select()
      .from(codex_runtime_capsules)
      .where(and(eq(codex_runtime_capsules.codexSessionId, capsule.codex_session_id), eq(codex_runtime_capsules.sequence, capsule.sequence)))
      .limit(1);
    const [existingForArtifact] = await this.db
      .select()
      .from(codex_runtime_capsules)
      .where(eq(codex_runtime_capsules.artifactRef, capsule.artifact_ref))
      .limit(1);
    const maxSequenceResult = await this.db.execute(sql<{ max_sequence: number | null }>`
      select max(sequence) as max_sequence from codex_runtime_capsules where codex_session_id = ${capsule.codex_session_id}
    `);
    const maxExistingSequence = Number(maxSequenceResult.rows[0]?.max_sequence ?? 0);
    if ((await this.getCodexRuntimeCapsule(capsule.id)) !== undefined || existingForSequence !== undefined || existingForArtifact !== undefined) {
    ...
    if (capsule.sequence <= maxExistingSequence) {
      throw new DomainError('workflow_invalid_transition', `workflow_invalid_transition: Codex runtime capsule ${capsule.id} sequence is stale`);
    }
    await this.db.insert(codex_runtime_capsules).values(toDbRecord(capsule, codex_runtime_capsules) as never);
  }
  ...
  private async terminalizeCodexSessionTurnUnlocked(input: TerminalizeCodexSessionTurnInput): Promise<{ session: CodexSession; turn: CodexSessionTurn }> {
    ...
    if (input.output_capsule !== undefined && input.output_capsule.created_from_turn_id !== input.turn_id) {
    ...
    const existingOutputCapsule = input.output_capsule === undefined ? undefined : await this.getCodexRuntimeCapsule(input.output_capsule.id);
    if (existingOutputCapsule !== undefined && existingOutputCapsule.created_from_turn_id !== input.turn_id) {
    ...
    if (
      ...
      !codexRuntimeCapsuleDurableIdentityMatches(existingOutputCapsule, input.output_capsule)
    ) {
      throw new DomainError('codex_runtime_capsule_stale', `codex_runtime_capsule_stale: Output capsule ${input.output_capsule.id} durable identity is stale`);
    }
    ...
    const updatedTurn: CodexSessionTurn = {
      ...
      ...(outputCapsule === undefined ? {} : { output_capsule_id: outputCapsule.id, output_capsule_digest: outputCapsule.digest }),
    ...
    const updatedSession: CodexSession = {
      ...
      ...(outputCapsule === undefined ? {} : { latest_capsule_id: outputCapsule.id, latest_capsule_digest: outputCapsule.digest }),
  ...
  private async terminalizeWorkflowExecutionUnlocked(input: TerminalizeWorkflowExecutionInput): Promise<TerminalizeWorkflowExecutionResult> {
    ...
    if (
      ...
      runtimeJob.workflow_id !== input.workflow_id ||
      runtimeJob.codex_session_id !== input.codex_session_id ||
      runtimeJob.codex_session_turn_id !== input.codex_session_turn_id ||
    ...
    const updatedRunSession: RunSession = {
      ...
      changed_files: (Array.isArray(terminalResult?.changed_files)
        ? terminalResult.changed_files
        : runSession.changed_files) as RunSession['changed_files'],
      check_results: (Array.isArray(terminalResult?.check_results)
        ? terminalResult.check_results
        : runSession.check_results) as RunSession['check_results'],
      artifacts: (Array.isArray(terminalResult?.execution_artifacts)
        ? terminalResult.execution_artifacts
        : runSession.artifacts) as RunSession['artifacts'],
  ...
  private async workflowExecutionSessionTerminalizationIsStale(input: TerminalizeWorkflowExecutionInput): Promise<boolean> {
    const sessionInput = input.codex_session_turn_terminalization;
    const session = await this.getCodexSession(sessionInput.session_id);
    const turn = await this.getCodexSessionTurn(sessionInput.turn_id);
    const lease = await this.getCodexSessionLease(sessionInput.lease_id);
    return (
      session === undefined ||
      turn === undefined ||
      lease === undefined ||
  ...
  private async createCodexSessionForkUnlocked(input: CreateCodexSessionForkInput): Promise<CodexSession> {
    ...
    const forkCapsule = input.forked_from_capsule_id === undefined ? undefined : await this.getCodexRuntimeCapsule(input.forked_from_capsule_id);
    if (
      workflow === undefined ||
      parent === undefined ||
    ...
    const forkTurnOutputCapsule =
      forkTurn?.output_capsule_id === undefined ? undefined : await this.getCodexRuntimeCapsule(forkTurn.output_capsule_id);
    if (
      forkTurn !== undefined &&
    ...
    const forkOutputCapsule =
      forkCapsule ??
      (forkTurn?.output_capsule_id === undefined || forkTurn.output_capsule_digest === undefined ? undefined : forkTurnOutputCapsule);
    ...
    const fork: CodexSession = {
      ...
      ...(forkLatestCapsule === undefined ? {} : { latest_capsule_id: forkLatestCapsule.id, latest_capsule_digest: forkLatestCapsule.digest }),
  ...
  async getCodexRuntimeJobWorkload(input: GetCodexRuntimeJobWorkloadInput): Promise<CodexRuntimeJob> {
    return this.db.transaction(async (tx) => {
      ...
      if (
        ...
        !workflowRunExecutionJobLineageMatches(runtimeJobFromDbRecord(bundle.job)) ||
  ...
  async pollCodexRuntimeJobs(input: PollCodexRuntimeJobsInput): Promise<CodexRuntimeJob[]> {
    return this.db.transaction(async (tx) => {
      ...
      const rows = await tx
        ...
        .limit(input.limit);
      return rows
        .map((row) => runtimeJobFromDbRecord(fromDbRecord<CodexRuntimeJobDbRecord>(row as Record<string, unknown>)))
        .filter(workflowRunExecutionJobLineageMatches);
    });
  }
  ...
  private async claimCodexLaunchTokenEnvelopeUnlocked(input: ClaimCodexLaunchTokenEnvelopeInput): Promise<CodexLaunchTokenEnvelope> {
    ...
    assertWorkflowRunExecutionJobLineageMatches(
      bundle.job === undefined ? undefined : runtimeJobFromDbRecord(bundle.job),
  ...
  private async materializeCodexRuntimeJobUnlocked(input: MaterializeCodexRuntimeJobInput): Promise<CodexLaunchMaterialization> {
    ...
    if (!canMaterializeDirectly(runtimeJobFromDbRecord(bundle.job))) {
      throw codexDenied(
        'codex_session_runner_unavailable',
    ...
    assertWorkflowRunExecutionJobLineageMatches(
      runtimeJobFromDbRecord(bundle.job),
      'codex_launch_materialization_denied',
  ...
  private async startCodexRuntimeJobUnlocked(input: StartCodexRuntimeJobInput): Promise<CodexRuntimeJob> {
    ...
    assertWorkflowRunExecutionJobLineageMatches(runtimeJobFromDbRecord(bundle.job), 'codex_runtime_job_unavailable', 'Codex runtime job start lineage was denied.');
  ...
  async saveRunSession(runSession: RunSession): Promise<void> {
    if (isActiveRunSessionStatus(runSession.status) && this.supportsSelect()) {
      ...
      if (runSession.codex_session_id !== undefined) {
        const [existingActiveForSession] = await this.db
          .select()
          .from(run_sessions)
          .where(
            and(
              eq(run_sessions.codexSessionId, runSession.codex_session_id),
        ...
        const activeForSession =
          existingActiveForSession === undefined ? undefined : fromDbRecord<RunSession>(existingActiveForSession as Record<string, unknown>);
        if (activeForSession !== undefined && activeForSession.id !== runSession.id) {
          throw new DomainError(
            'workflow_execution_already_running',
            `workflow_execution_already_running: Codex session ${runSession.codex_session_id} already has active run ${activeForSession.id}`,
          );
        }
      }
    }
